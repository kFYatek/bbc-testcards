...
def _add_input_file(subplot, plotscale, input_file: str, input_colorspace: common.ColorSpace,
    ...
    if channel == 'Y':
        ...
        assert output_colorspace is not common.ColorSpace.YUV
        data = numpy.matvec(output_colorspace.to_rgb_matrix, data, axes=[(0, 1), 2, 2])
        if channel == 'R':
            data = data[:, :, 0]
        elif channel == 'G':
            data = data[:, :, 1]
    ...
    return data.shape, xlim_changed, change_line
...
def _main(*args):
    ...
    for input_file in args.input_files:
        ...
        subplot.callbacks.connect('xlim_changed', f_xlim_changed)
        change_lines.append(f_change_line)

    slider_frame = matplotlib.pyplot.axes((0.1, 0.1, 0.8, 0.03))
    slider = matplotlib.widgets.Slider(slider_frame, 'Line', 0, shape[0] - 1, valinit=0,
                                       valfmt='%d')

    def slider_update(val):
        lineno = int(numpy.floor(slider.val))
        for update in change_lines:
            update(lineno)
        subplot.relim()
        matplotlib.pyplot.draw()
        subplot.callbacks.process('xlim_changed', None)
