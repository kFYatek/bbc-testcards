...
env CARD=4 SCALER=Lanczos vspipe "$SCRIPTDIR/extract.vpy" - | "$SCRIPTDIR/convert.py" > "$OUTDIR/TestCardA.$EXT"
env CARD=5 SCALER=Lanczos vspipe "$SCRIPTDIR/extract.vpy" - | "$SCRIPTDIR/convert.py" > "$OUTDIR/TestCardB.$EXT"

# These test cards look like JPEGs, but they were originally optical, so just scale them down
env CARD=2 SCALER=Lanczos vspipe "$SCRIPTDIR/extract.vpy" - | "$SCRIPTDIR/convert.py" > "$OUTDIR/TuningSignal.$EXT"
env CARD=6 SCALER=Lanczos vspipe "$SCRIPTDIR/extract.vpy" - | "$SCRIPTDIR/convert.py" > "$OUTDIR/TestCardC.$EXT"
env CARD=7 SCALER=Lanczos vspipe "$SCRIPTDIR/extract.vpy" - | "$SCRIPTDIR/convert.py" > "$OUTDIR/TestCardD.$EXT"
env CARD=8 SCALER=Lanczos vspipe "$SCRIPTDIR/extract.vpy" - | "$SCRIPTDIR/convert.py" > "$OUTDIR/TestCardFOpt.$EXT"
